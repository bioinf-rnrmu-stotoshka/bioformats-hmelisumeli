...
# Требования: numpy, matplotlib, seaborn, plotly
# Установка: pip install numpy matplotlib seaborn plotly
...
# НАСТРОЙКА: ЗДЕСЬ ПУТЬ К FASTQ ФАЙЛУ И ПАПКЕ ДЛЯ ГРАФИКОВ
FASTQ_FILE = "C:/Users/PC/Documents/fastq_project/SRR35776190.fastq"
OUTPUT_DIR = "C:/Users/PC/Documents/fastq_project/graphs"

# 1. БАЗОВЫЕ КЛАССЫ (абстрактная иерархия из задания)

class Record:
    # Класс для хранения одной записи FASTQ
...
class SequenceReader(Reader):
    # Абстрактный класс для читателей последовательностей
    @abstractmethod
...
# 2. ОСНОВНОЙ КЛАСС ДЛЯ ЧТЕНИЯ FASTQ (с генераторами)

class FastqReader(SequenceReader):
    # Класс для чтения FASTQ файлов
    # ОПТИМИЗАЦИЯ: использует генераторы для работы с большими файлами
    def __init__(self, filename: str):
        self.filename = filename
    ...
    def read(self) -> Iterator[Record]:
        # ГЕНЕРАТОР для чтения FASTQ файла
        ...
        while True:
            ...
            plus = self.file_handle.readline().strip()
    ...
    @staticmethod
    def _quality_string_to_scores(quality_str: str, phred_offset: int = 33) -> List[int]:
        return [ord(char) - phred_offset for char in quality_str]

# 3. АНАЛИЗАТОР FASTQ

class FastqAnalyzer:
    # Класс для анализа FASTQ файлов
    ...
    def calculate_per_base_quality(self) -> Tuple[List[float], List[List[int]]]:
        position_qualities = defaultdict(list)
        with self.reader:
            for record in self.reader.read():
                qualities = FastqReader._quality_string_to_scores(record.quality)
    ...
    def calculate_per_base_content(self) -> Dict[str, List[float]]:
        ...
        for pos in range(max_pos + 1):
            ...
            if total > 0:
                for base in ['A', 'C', 'G', 'T']:
                    result[base].append((counts[base] / total) * 100)
            else:
                for base in ['A', 'C', 'G', 'T']:
                    result[base].append(0)
        return result
    ...
    def plot_per_base_quality_matplotlib(self):
        ...
        output_path = os.path.join(self.output_dir, 'per_base_quality_matplotlib.png')
    ...
    def plot_per_base_quality_seaborn(self):
        ...
        output_path = os.path.join(self.output_dir, 'per_base_quality_seaborn.png')
        plt.savefig(output_path, dpi=300)
        plt.close()
        print(f"График сохранен: {output_path}")
    ...
    def plot_per_base_quality_plotly(self):
        ...
        output_path = os.path.join(self.output_dir, 'per_base_quality_plotly.html')
    ...
    def plot_per_base_content(self):
        ...
        output_path = os.path.join(self.output_dir, 'per_base_content.png')
        plt.savefig(output_path, dpi=300)
        plt.close()
        print(f"График сохранен: {output_path}")
    ...
    def plot_sequence_length_distribution(self):
        ...
        output_path = os.path.join(self.output_dir, 'sequence_length_distribution.png')
        plt.savefig(output_path, dpi=300)
        plt.close()
        print(f"График сохранен: {output_path}")
...
if __name__ == "__main__":
    print(f"Анализ FASTQ файла: {FASTQ_FILE}")
    
    if not os.path.isfile(FASTQ_FILE):
        print(f"Ошибка: файл не найден: {FASTQ_FILE}")
        exit(1)
    
    if not os.path.isdir(OUTPUT_DIR):
        print(f"Ошибка: папка graphs не найдена: {OUTPUT_DIR}")
        exit(1)
    
    print(f"Папка для графиков: {OUTPUT_DIR}")
    
    analyzer = FastqAnalyzer(FASTQ_FILE, output_dir=OUTPUT_DIR)
    ...
    print(f"\nАнализ завершен! Графики в папке: {OUTPUT_DIR}")